...
public class BitmovinYospacePlayer extends BitmovinPlayer {
    ...
    private EventListener<Session> sessionEventListener = new EventListener<Session>() {
        ...
        public void handle(Event<Session> event) {
            ...
            BitLog.d("Session State: " + session.getState().toString() + " Result Code: " + session.getResultCode());
            switch (session.getState()) {
                case INITIALISED:
                    session.addAnalyticListener(analyticEventListener);
                    BitLog.i("Yospace Session Initialized. Session Player Url: " + session.getPlayerUrl());
    ...
    private OnSourceLoadedListener onSourceLoadedListener = new OnSourceLoadedListener() {
        ...
        public void onSourceLoaded(SourceLoadedEvent sourceLoadedEvent) {
            ...
            if (session instanceof SessionNonLinear) {
                ...
                if (adBreaks != null) {
                    BitLog.d("Ad Breaks: " + adBreaks.toString());
                        ...
                        if (isLiveAdPaused) {
                            if (activeAdBreak != null) {
                                ...
                                if (currentTime - pausedTime > adBreakAbsEnd - pausedTime) {
                                    ...
                                    yospaceEventEmitter.emit(new AdSkippedEvent(activeAd));
                                }
                            }
                        }
                        isLiveAdPaused = false;
    ...
    private IEventHandler adStartedListener = new IEventHandler() {
        ...
        public void handleEvent(Map<String, ?> data) {
            ...
            if (activeAd != null) {
                adStartedEvent = YospaceUtil.createAdStartEvent(AdSourceType.UNKNOWN, activeAd.getClickThroughUrl(), activeAd.getSequence(), activeAd.getDuration(), activeAd.getRelativeStart(), "position", 0, activeAd.isTruex(), activeAd);
            } else {
                adStartedEvent = YospaceUtil.createAdStartEvent(AdSourceType.UNKNOWN, "", 0, 0, 0, "0", 0, true, null);
            }
            BitLog.d("Emitting AdBreakStartedEvent");
            yospaceEventEmitter.emit(new AdBreakStartedEvent(getActiveAdBreak()));
    ...
    private AnalyticEventListener analyticEventListener = new AnalyticEventListener() {
            ...
            if (adFree) {
                ...
                if (!isTruexRendering) {
                    ...
                    if (isLive()) {
                        ...
                        liveAdBreak = new AdBreak(adId, absoluteTime, adBreak.getDuration() / 1000.0, absoluteTime, absoluteTime + adBreak.getDuration() / 1000.0, 0);
                    }
                    AdBreak activeAdBreak = getActiveAdBreak();
                    handler.post(new Runnable() {
                        @Override
        ...
        public void onTrackingUrlCalled(Advert advert, String s, String s1) {
            BitLog.d("OnTrackingUrlCalled: " + s);
        }

        @Override
        public void onVastReceived(VastPayload vastPayload) {
            BitLog.d("OnVastReceived: " + vastPayload.getRaw());
        }
    };
}
